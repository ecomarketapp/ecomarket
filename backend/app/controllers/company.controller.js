...
module.exports = {
  ...
  createCompany: async (req, res) => {
    ...
    try {
      ...
      const company_check = await Company.findOne({ wallet_address: wallet });
      if (!company_check) {
  ...
  getOneCompany: async (req, res) => {
    ...
    try {
      ...
      const company = await Company.findOne({ wallet_address: wallet }).exec();
      console.log(company);
      return res.json({ status: true, data: company });
    } catch (error) {
      return res.status(500).send({
  ...
  companyAmountLocked: async (req, res) => {
    ...
    try {
      ...
      const delivered_amounts = [
        ...new Set(deliveries.map((delivery) => delivery.delivery_amount)),
      ];
