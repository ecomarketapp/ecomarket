...
const checkDeliveryExpiry = async (deliveryId) => {
    ...
    try {
        ...
        if (delivery.delivery_status === "APPROVED" && delivery.approved_at) {
            const time_has_elapsed = dayjs().isAfter(
                dayjs(delivery.approved_at).add(
                    process.env.DELIVERY_EXPIRY_DEFAULT,
                    "hour"
                )
            );
            if (time_has_elapsed) {
                delivery.delivery_status = "EXPIRED";
                delivery.expired_at = dayjs(delivery.approved_at).add(
                    process.env.DELIVERY_EXPIRY_DEFAULT,
                    "hour"
                );
        ...
        throw new Error(
            error.message ||
                `There was an error checking this delivery's Expiry status`
        );
    }
};
...
const checkCompany = async (requestId, company_wallet_address) => {
    try {
        const valid_request = await Request.findById(requestId)
            ...
            .exec();
        if (
            valid_request.company &&
            valid_request.company.wallet_address === company_wallet_address
        ) {
            return true;
        } else {
            return false;
        ...
        throw new Error(
            error.message ||
                `There was an error checking this delivery's related company`
        );
    }
};
...
const checkCollector = async (deliveryId, collector_wallet_address) => {
    try {
        const delivery = await Delivery.findOne({ _id: deliveryId }).populate({
            path: "collector",
            model: Collector,
            select: "_id, wallet_address",
        });
        ...
        if (
            delivery.collector &&
            delivery.collector.wallet_address === collector_wallet_address
        ) {
            return true;
        } else {
            return false;
        ...
        throw new Error(
            error.message ||
                `There was an error checking this delivery's related collector`
        );
    }
};
...
module.exports = {
    createDelivery: async (req, res) => {
        ...
        try {
            ...
            if (Array.isArray(deliveries_for_request)) {
                const completed_delivery_qties = [
                    ...new Set(
                        deliveries_for_request.map(
                            (delivery) => delivery.delivery_size
                        )
                    ),
                ];
                if (completed_delivery_qties.length) {
                    total_delivery_qty = completed_delivery_qties.reduce(
                        (a, b) => a + b
                    );
                }
            } else {
    ...
    approveDelivery: async (req, res) => {
        ...
        const { approver_signature, approver_wallet_address, requestId } =
            req.body;
        try {
            // find delivery
            ...
            const valid_company = await checkCompany(
                requestId,
    ...
    completeDelivery: async (req, res) => {
        ...
        try {
            ...
            const valid_collector = await checkCollector(
                deliveryId,
                collector_wallet_address
            );
            if (!valid_collector) {
            ...
            if (delivery_size && delivery_proof) {
                ...
                delivery.delivery_amount =
                    request.amount_per_unit * delivery_size;
                delivery.delivery_proof = delivery_proof;
                delivery.delivery_status = "DELIVERED";
                delivery.delivered_at = new Date();
                delivery = await delivery.save();

                delivery = await Delivery.findById(delivery.id).populate({
                    path: "collector",
                    model: Collector,
                });
                delivery = delivery.toJSON();
    ...
    claimRewardForDelivery: async (req, res) => {
        ...
        try {
            ...
            const cooloff_period_elapsed = dayjs().isAfter(
                dayjs(delivery.delivered_at).add(
                    process.env.COOL_OFF_PERIOD,
                    "hour"
                )
            );
            ...
            return res.status(500).json({
                status: false,
                message:
                    error.message || `There was an error claiming this reward`,
            });
        }
    },
